...
from coatmenu.core import runner
...
class MenuPopup(QWidget):
    ...
    def _rebuild_rows(self) -> None:
        """Lay out every row (the geometry the painter and the hit test share)."""
...
def run_item(item: MenuItem) -> None:
    """Execute one menu entry (the implementation lives in ``core.runner``)."""
    runner.run_item(item)
